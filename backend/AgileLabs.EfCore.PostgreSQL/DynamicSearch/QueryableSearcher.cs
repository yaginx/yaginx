...
/// <summary>
///     查询方法，提供给 <see cref="FrameworkNetCore.EFEx.DynamicSearch.QueryableExtensions" />调用
...
internal class QueryableSearcher<T>
{
        ...
        new Dictionary<QueryMethod, Func<Expression, Expression, Expression>>
        {
            ...
            },
            {
                ...
                (left, right) =>
                {
                    ...
                    if (right.Type != typeof(string)) return null;
                    if (right.NodeType != ExpressionType.Constant) return null;
                    var likePattern =  ((ConstantExpression)right).Value.ToString();
                    likePattern = "%" + likePattern + "%";
                    var likePatternConstantExp = Expression.Constant(likePattern);
                    ...
                    return Expression.Call(null, like, functions,  left,  likePatternConstantExp);
                }
            },
            {
                QueryMethod.StdIn,
        ...
        set
        {
            ...
            foreach (var it in value)
            {
                if (it.Value == null || it.Value.ToString() == "")
                {
                    ...
                    if (Pinfo == null) throw new Exception(it.Field + " No target object exists, unable to construct query condition");
                    if (Pinfo.PropertyType.IsPrimitive) throw new Exception(it.Field + " Belongs to a simple type, there are no null or NULL values, and query criteria cannot be constructed");//简单类型为空直接跳过
                    newItems.Add(it);
                }
                else
                    newItems.Add(it);
    ...
    private Expression GetExpressoinBody(ParameterExpression param, IEnumerable<ConditionItem> items)
    {
        ...
        if (andList.Count() != 0)
        {
            var exp = GetGroupExpression(param, andList, Expression.AndAlso);
            if (exp != null)
                list.Add(exp);
        }
        ...
        foreach (var orAnd in orAndGrups)
        {
            ...
            if (orAnd.Count() != 0)
            {
                var exp = GetGroupExpression(param, orAnd, Expression.AndAlso);
                if (exp == null)
                    continue;

                childAndExp.Add(orAnd.Key, exp);
    ...
    private Expression GetExpression(ParameterExpression param, ConditionItem item)
    {
        ...
        if (item.Method == QueryMethod.AutoDetect)
        {
            switch (exp.ReturnType.Name)
            {
                case nameof(String):
                    item.Method = QueryMethod.Contains;
                    break;
                case nameof(Int64):
