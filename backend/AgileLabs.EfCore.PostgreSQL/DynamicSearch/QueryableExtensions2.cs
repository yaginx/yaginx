...
public class Pagination<T> where T : class
{
        ...
        out int count, bool isGetTotalCount = true)
    {
        ...
        IQueryable<T> iOrderedQueryable = OrderedQueryable(query, sort, dir);
        // skipCount 必须大于等于0
    ...
    /// <returns></returns>
    private static IQueryable<T> OrderedQueryable(IQueryable<T> query, string sort, string dir)
    {
        ...
        if (string.IsNullOrEmpty(dir))
        {
            if (!string.IsNullOrEmpty(sort))
            {
                ...
                for (int i = 0; i < sorts.Length; i++)
                {
                    ...
                    if (i == 0)
                        iOrderedQueryable = query.OrderBy(sortName + " " + sortDir);
                    else
                        iOrderedQueryable = iOrderedQueryable.ThenBy(sortName + " " + sortDir);
                }
            }
            else
            {
    ...
    /// <param name="count">总纪录数</param>
    /// <param name="isGetTotalCount">是否获取总页数，默认是true</param>
    /// <param name="filterFunc">外部过滤函数</param>
    /// <param name="filterArgs">过滤参数</param>
    /// <returns></returns>
    public static IEnumerable<T> PageList<A>(IQueryable<T> query, int skipCount, int pageSize, string sort, string dir,
        out int count, bool isGetTotalCount = true, Func<List<T>, A, List<T>> filterFunc = null, A filterArgs = default)
    {
        IQueryable<T> iOrderedQueryable = OrderedQueryable(query, sort, dir);
        //执行数据查询
        var queryResult = iOrderedQueryable.AsNoTracking().ToList();
        //回调方法再次过滤数据
        if (filterFunc != null)
        {
            queryResult = filterFunc(queryResult, filterArgs);
        }
        ...
        if (pageSize > 0)
        {
            ...
            return queryResult.Skip(skipCount).Take(pageSize);
        }
        else
        {
            return query.Take(0);
